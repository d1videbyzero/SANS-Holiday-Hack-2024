...
public abstract class NodeCoordinator extends LookaheadCapablePlaceable implements Measurable, LayoutCoordinates, OwnerScope, Function1<Canvas, Unit> {
    ...
    private static final float[] tmpMatrix = Matrix.m2856constructorimpl$default(null, 1, null);
    private static final HitTestSource PointerInputSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$PointerInputSource$1
        ...
        public void mo4077childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            Intrinsics.checkNotNullParameter(layoutNode, "layoutNode");
            Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
            layoutNode.m4007hitTestM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw, reason: not valid java name */
        public int mo4078entityTypeOLwlOKw() {
            return NodeKind.m4082constructorimpl(16);
        }
        ...
        public boolean interceptOutOfBoundsChildEvents(Modifier.Node node) {
            ...
            int m4082constructorimpl = NodeKind.m4082constructorimpl(16);
            ...
            while (node != 0) {
                ...
                } else if ((node.getKindSet() & m4082constructorimpl) != 0 && (node instanceof DelegatingNode)) {
                    Modifier.Node delegate = node.getDelegate();
                    int i = 0;
                    node = node;
                    while (delegate != null) {
                        if ((delegate.getKindSet() & m4082constructorimpl) != 0) {
                            i++;
                            if (i == 1) {
                                node = delegate;
                            } else {
    ...
    private static final HitTestSource SemanticsSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$SemanticsSource$1
        ...
        public void mo4077childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            Intrinsics.checkNotNullParameter(layoutNode, "layoutNode");
            Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
            layoutNode.m4008hitTestSemanticsM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw */
        public int mo4078entityTypeOLwlOKw() {
            return NodeKind.m4082constructorimpl(8);
        }
    };
    ...
    public interface HitTestSource {
        /* renamed from: childHitTest-YqVAtuI */
        void mo4077childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer);

        /* renamed from: entityType-OLwlOKw */
        int mo4078entityTypeOLwlOKw();
    ...
    public NodeCoordinator(LayoutNode layoutNode) {
        ...
        this.position = IntOffset.INSTANCE.m5006getZeronOccac();
    ...
    public final /* synthetic */ <T> void m4075visitNodesaLcG6gQ(int type, Function1<? super T, Unit> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        boolean m4091getIncludeSelfInTraversalH91voCI = NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m4091getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m4091getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & type) != 0; headNode = headNode.getChild()) {
    ...
    private final boolean m4055hasNodeH91voCI(int type) {
        Modifier.Node headNode = headNode(NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(type));
        return headNode != null && DelegatableNodeKt.m3979has64DMado(headNode, type);
    }

    /* renamed from: head-H91voCI, reason: not valid java name */
    public final Modifier.Node m4068headH91voCI(int type) {
        boolean m4091getIncludeSelfInTraversalH91voCI = NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m4091getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return null;
    ...
    public final long mo3875getSizeYbymL2g() {
    ...
    public void replace$ui_release() {
        mo3869placeAtf8xVGno(getPosition(), this.zIndex, this.layerBlock);
    }
    ...
    protected void onMeasureResultChanged(int width, int height) {
        ...
        if (ownedLayer != null) {
            ownedLayer.mo4131resizeozmzZPI(IntSizeKt.IntSize(width, height));
        } else {
            NodeCoordinator nodeCoordinator = this.wrappedBy;
            if (nodeCoordinator != null) {
        ...
        m3923setMeasuredSizeozmzZPI(IntSizeKt.IntSize(width, height));
        updateLayerParameters(false);
        int m4082constructorimpl = NodeKind.m4082constructorimpl(4);
        boolean m4091getIncludeSelfInTraversalH91voCI = NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(m4082constructorimpl);
        Modifier.Node tail = getTail();
        if (m4091getIncludeSelfInTraversalH91voCI || (tail = tail.getParent()) != null) {
            for (Modifier.Node headNode = headNode(m4091getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & m4082constructorimpl) != 0; headNode = headNode.getChild()) {
                if ((headNode.getKindSet() & m4082constructorimpl) != 0) {
                    DelegatingNode delegatingNode = headNode;
                    MutableVector mutableVector = null;
                    while (delegatingNode != 0) {
                        if (delegatingNode instanceof DrawModifierNode) {
                            ((DrawModifierNode) delegatingNode).onMeasureResultChanged();
                        } else if ((delegatingNode.getKindSet() & m4082constructorimpl) != 0 && (delegatingNode instanceof DelegatingNode)) {
    ...
    protected void m4073setPositiongyyYBs(long j) {
        this.position = j;
    }
    ...
    public Object getParentData() {
        if (!getLayoutNode().getNodes().m4045hasH91voCI$ui_release(NodeKind.m4082constructorimpl(64))) {
            return null;
        }
        getTail();
        Ref.ObjectRef objectRef = new Ref.ObjectRef();
        for (Modifier.Node tail = getLayoutNode().getNodes().getTail(); tail != null; tail = tail.getParent()) {
            if ((NodeKind.m4082constructorimpl(64) & tail.getKindSet()) != 0) {
                int m4082constructorimpl = NodeKind.m4082constructorimpl(64);
                MutableVector mutableVector = null;
    ...
    public final long m4066getLastMeasurementConstraintsmsEJaDk$ui_release() {
        return getMeasurementConstraints();
    }

    /* renamed from: performingMeasure-K40F9xA, reason: not valid java name */
    protected final Placeable m4071performingMeasureK40F9xA(long constraints, Function0<? extends Placeable> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        m3924setMeasurementConstraintsBRTryo0(constraints);
        return block.invoke();
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    protected void mo3869placeAtf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        m4059placeSelff8xVGno(position, zIndex, layerBlock);
    }

    /* renamed from: placeSelf-f8xVGno, reason: not valid java name */
    private final void m4059placeSelff8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        updateLayerBlock$default(this, layerBlock, false, 2, null);
        if (!IntOffset.m4995equalsimpl0(getPosition(), position)) {
            m4073setPositiongyyYBs(position);
            getLayoutNode().getLayoutDelegate().getMeasurePassDelegate().notifyChildrenUsingCoordinatesWhilePlacing();
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4130movegyyYBs(position);
            } else {
                NodeCoordinator nodeCoordinator = this.wrappedBy;
                if (nodeCoordinator != null) {
                    nodeCoordinator.invalidateLayer();
    ...
    public final void m4072placeSelfApparentToRealOffsetf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        long j = getApparentToRealOffset();
        m4059placeSelff8xVGno(IntOffsetKt.IntOffset(IntOffset.m4996getXimpl(position) + IntOffset.m4996getXimpl(j), IntOffset.m4997getYimpl(position) + IntOffset.m4997getYimpl(j)), zIndex, layerBlock);
    }
    ...
    public final void draw(Canvas canvas) {
        ...
        float m4996getXimpl = IntOffset.m4996getXimpl(getPosition());
        float m4997getYimpl = IntOffset.m4997getYimpl(getPosition());
        canvas.translate(m4996getXimpl, m4997getYimpl);
    ...
    public final void updateLayerBlock(Function1<? super GraphicsLayerScope, Unit> layerBlock, boolean forceUpdateLayerParameters) {
        ...
        if (isAttached() && layerBlock != null) {
            ...
            createLayer.mo4131resizeozmzZPI(getMeasuredSize());
            createLayer.mo4130movegyyYBs(getPosition());
    ...
    private final void updateLayerParameters(boolean invokeOnLayoutChange) {
        ...
        if (ownedLayer != null) {
            ...
            reusableGraphicsLayerScope.m2936setSizeuvyYCjk(IntSizeKt.m5048toSizeozmzZPI(mo3875getSizeYbymL2g()));
    ...
    public final long m4067getMinimumTouchTargetSizeNHjbRc() {
        return this.layerDensity.mo328toSizeXkaWNTQ(getLayoutNode().getViewConfiguration().mo4011getMinimumTouchTargetSizeMYxV2XQ());
    }

    /* renamed from: hitTest-YqVAtuI, reason: not valid java name */
    public final void m4069hitTestYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        Modifier.Node m4068headH91voCI = m4068headH91voCI(hitTestSource.mo4078entityTypeOLwlOKw());
        if (!m4076withinLayerBoundsk4lQ0M(pointerPosition)) {
            if (isTouchEvent) {
                float m4064distanceInMinimumTouchTargettz77jQw = m4064distanceInMinimumTouchTargettz77jQw(pointerPosition, m4067getMinimumTouchTargetSizeNHjbRc());
                if (Float.isInfinite(m4064distanceInMinimumTouchTargettz77jQw) || Float.isNaN(m4064distanceInMinimumTouchTargettz77jQw) || !hitTestResult.isHitInMinimumTouchTargetBetter(m4064distanceInMinimumTouchTargettz77jQw, false)) {
                    return;
                }
                m4057hitNearJHbHoSQ(m4068headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, false, m4064distanceInMinimumTouchTargettz77jQw);
                return;
            }
            return;
        }
        if (m4068headH91voCI == null) {
            mo4002hitTestChildYqVAtuI(hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        if (m4070isPointerInBoundsk4lQ0M(pointerPosition)) {
            m4056hit1hIXUjU(m4068headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        float m4064distanceInMinimumTouchTargettz77jQw2 = !isTouchEvent ? Float.POSITIVE_INFINITY : m4064distanceInMinimumTouchTargettz77jQw(pointerPosition, m4067getMinimumTouchTargetSizeNHjbRc());
        if (!Float.isInfinite(m4064distanceInMinimumTouchTargettz77jQw2) && !Float.isNaN(m4064distanceInMinimumTouchTargettz77jQw2)) {
            if (hitTestResult.isHitInMinimumTouchTargetBetter(m4064distanceInMinimumTouchTargettz77jQw2, isInLayer)) {
                m4057hitNearJHbHoSQ(m4068headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m4064distanceInMinimumTouchTargettz77jQw2);
                return;
            }
        }
        m4060speculativeHitJHbHoSQ(m4068headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m4064distanceInMinimumTouchTargettz77jQw2);
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: hit-1hIXUjU, reason: not valid java name */
    public final void m4056hit1hIXUjU(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2) {
        if (node == null) {
            mo4002hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hit(node, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m4080nextUntilhw7D004;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4080nextUntilhw7D004 = NodeCoordinatorKt.m4080nextUntilhw7D004(node, hitTestSource.mo4078entityTypeOLwlOKw(), NodeKind.m4082constructorimpl(2));
                    nodeCoordinator.m4056hit1hIXUjU(m4080nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2);
                }
            });
        }
    }
    ...
    public final void m4057hitNearJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        if (node == null) {
            mo4002hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hitInMinimumTouchTarget(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hitNear$1
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4080nextUntilhw7D004 = NodeCoordinatorKt.m4080nextUntilhw7D004(node, hitTestSource.mo4078entityTypeOLwlOKw(), NodeKind.m4082constructorimpl(2));
                    nodeCoordinator.m4057hitNearJHbHoSQ(m4080nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public final void m4060speculativeHitJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        Modifier.Node m4080nextUntilhw7D004;
        if (node == null) {
            mo4002hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (!hitTestSource.interceptOutOfBoundsChildEvents(node)) {
            m4080nextUntilhw7D004 = NodeCoordinatorKt.m4080nextUntilhw7D004(node, hitTestSource.mo4078entityTypeOLwlOKw(), NodeKind.m4082constructorimpl(2));
            m4060speculativeHitJHbHoSQ(m4080nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
        } else {
            hitTestResult.speculativeHit(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$speculativeHit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m4080nextUntilhw7D0042;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m4080nextUntilhw7D0042 = NodeCoordinatorKt.m4080nextUntilhw7D004(node, hitTestSource.mo4078entityTypeOLwlOKw(), NodeKind.m4082constructorimpl(2));
                    nodeCoordinator.m4060speculativeHitJHbHoSQ(m4080nextUntilhw7D0042, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public void mo4002hitTestChildYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Intrinsics.checkNotNullParameter(hitTestSource, "hitTestSource");
        Intrinsics.checkNotNullParameter(hitTestResult, "hitTestResult");
        NodeCoordinator nodeCoordinator = this.wrapped;
        if (nodeCoordinator != null) {
            nodeCoordinator.m4069hitTestYqVAtuI(hitTestSource, nodeCoordinator.m4065fromParentPositionMKHz9U(pointerPosition), hitTestResult, isTouchEvent, isInLayer);
        }
    }
    ...
    public final Rect touchBoundsInRoot() {
        ...
        long m4063calculateMinimumTouchTargetPaddingE7KxVPU = m4063calculateMinimumTouchTargetPaddingE7KxVPU(m4067getMinimumTouchTargetSizeNHjbRc());
        rectCache.setLeft(-Size.m2460getWidthimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setTop(-Size.m2457getHeightimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setRight(getMeasuredWidth() + Size.m2460getWidthimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setBottom(getMeasuredHeight() + Size.m2457getHeightimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU));
    ...
    public long mo3880windowToLocalMKHz9U(long relativeToWindow) {
        ...
        LayoutCoordinates findRootCoordinates = LayoutCoordinatesKt.findRootCoordinates(this);
        return mo3876localPositionOfR5De75A(findRootCoordinates, Offset.m2395minusMKHz9U(LayoutNodeKt.requireOwner(getLayoutNode()).mo4134calculateLocalPositionMKHz9U(relativeToWindow), LayoutCoordinatesKt.positionInRoot(findRootCoordinates)));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localToWindow-MK-Hz9U */
    public long mo3878localToWindowMKHz9U(long relativeToLocal) {
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo4135calculatePositionInWindowMKHz9U(mo3877localToRootMKHz9U(relativeToLocal));
    }
    ...
    public long mo3876localPositionOfR5De75A(LayoutCoordinates sourceCoordinates, long relativeToSource) {
        ...
        if (sourceCoordinates instanceof LookaheadLayoutCoordinatesImpl) {
            return Offset.m2400unaryMinusF1C5BW0(sourceCoordinates.mo3876localPositionOfR5De75A(this, Offset.m2400unaryMinusF1C5BW0(relativeToSource)));
        }
        ...
        while (coordinator != findCommonAncestor$ui_release) {
            relativeToSource = coordinator.m4074toParentPositionMKHz9U(relativeToSource);
            coordinator = coordinator.wrappedBy;
            Intrinsics.checkNotNull(coordinator);
        }
        return m4054ancestorToLocalR5De75A(findCommonAncestor$ui_release, relativeToSource);
    }
    ...
    public void mo3879transformFromEL8BTi8(LayoutCoordinates sourceCoordinates, float[] matrix) {
        ...
        coordinator.onCoordinatesUsed$ui_release();
        NodeCoordinator findCommonAncestor$ui_release = findCommonAncestor$ui_release(coordinator);
        Matrix.m2865resetimpl(matrix);
        coordinator.m4062transformToAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
        m4061transformFromAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
    }

    /* renamed from: transformToAncestor-EL8BTi8, reason: not valid java name */
    private final void m4062transformToAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        NodeCoordinator nodeCoordinator = this;
        while (!Intrinsics.areEqual(nodeCoordinator, ancestor)) {
            OwnedLayer ownedLayer = nodeCoordinator.layer;
            if (ownedLayer != null) {
                ownedLayer.mo4132transform58bKbWc(matrix);
            }
            if (!IntOffset.m4995equalsimpl0(nodeCoordinator.getPosition(), IntOffset.INSTANCE.m5006getZeronOccac())) {
                float[] fArr = tmpMatrix;
                Matrix.m2865resetimpl(fArr);
                Matrix.m2876translateimpl$default(fArr, IntOffset.m4996getXimpl(r1), IntOffset.m4997getYimpl(r1), 0.0f, 4, null);
                Matrix.m2873timesAssign58bKbWc(matrix, fArr);
            }
            nodeCoordinator = nodeCoordinator.wrappedBy;
            Intrinsics.checkNotNull(nodeCoordinator);
        }
    ...
    private final void m4061transformFromAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        if (Intrinsics.areEqual(ancestor, this)) {
            return;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        Intrinsics.checkNotNull(nodeCoordinator);
        nodeCoordinator.m4061transformFromAncestorEL8BTi8(ancestor, matrix);
        if (!IntOffset.m4995equalsimpl0(getPosition(), IntOffset.INSTANCE.m5006getZeronOccac())) {
            float[] fArr = tmpMatrix;
            Matrix.m2865resetimpl(fArr);
            Matrix.m2876translateimpl$default(fArr, -IntOffset.m4996getXimpl(getPosition()), -IntOffset.m4997getYimpl(getPosition()), 0.0f, 4, null);
            Matrix.m2873timesAssign58bKbWc(matrix, fArr);
        }
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mo4127inverseTransform58bKbWc(matrix);
        }
    }
    ...
    public Rect localBoundingBoxOf(LayoutCoordinates sourceCoordinates, boolean clipBounds) {
        ...
        rectCache.setRight(IntSize.m5038getWidthimpl(sourceCoordinates.mo3875getSizeYbymL2g()));
        rectCache.setBottom(IntSize.m5037getHeightimpl(sourceCoordinates.mo3875getSizeYbymL2g()));
        while (coordinator != findCommonAncestor$ui_release) {
    ...
    private final long m4054ancestorToLocalR5De75A(NodeCoordinator ancestor, long offset) {
        if (ancestor == this) {
            return offset;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        if (nodeCoordinator == null || Intrinsics.areEqual(ancestor, nodeCoordinator)) {
            return m4065fromParentPositionMKHz9U(offset);
        }
        return m4065fromParentPositionMKHz9U(nodeCoordinator.m4054ancestorToLocalR5De75A(ancestor, offset));
    }
    ...
    public long mo3877localToRootMKHz9U(long relativeToLocal) {
        ...
        for (NodeCoordinator nodeCoordinator = this; nodeCoordinator != null; nodeCoordinator = nodeCoordinator.wrappedBy) {
            relativeToLocal = nodeCoordinator.m4074toParentPositionMKHz9U(relativeToLocal);
        }
    ...
    public long m4074toParentPositionMKHz9U(long position) {
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            position = ownedLayer.mo4129mapOffset8S9VItk(position, false);
        }
        return IntOffsetKt.m5010plusNvtHpc(position, getPosition());
    }

    /* renamed from: fromParentPosition-MK-Hz9U, reason: not valid java name */
    public long m4065fromParentPositionMKHz9U(long position) {
        long m5008minusNvtHpc = IntOffsetKt.m5008minusNvtHpc(position, getPosition());
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer != null ? ownedLayer.mo4129mapOffset8S9VItk(m5008minusNvtHpc, true) : m5008minusNvtHpc;
    }

    protected final void drawBorder(Canvas canvas, Paint paint) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(paint, "paint");
        canvas.drawRect(new Rect(0.5f, 0.5f, IntSize.m5038getWidthimpl(getMeasuredSize()) - 0.5f, IntSize.m5037getHeightimpl(getMeasuredSize()) - 0.5f), paint);
    }
    ...
    public final void rectInParent$ui_release(MutableRect bounds, boolean clipBounds, boolean clipToMinimumTouchTargetSize) {
        ...
        if (ownedLayer != null) {
            if (this.isClipping) {
                if (clipToMinimumTouchTargetSize) {
                    long m4067getMinimumTouchTargetSizeNHjbRc = m4067getMinimumTouchTargetSizeNHjbRc();
                    float m2460getWidthimpl = Size.m2460getWidthimpl(m4067getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    float m2457getHeightimpl = Size.m2457getHeightimpl(m4067getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    bounds.intersect(-m2460getWidthimpl, -m2457getHeightimpl, IntSize.m5038getWidthimpl(mo3875getSizeYbymL2g()) + m2460getWidthimpl, IntSize.m5037getHeightimpl(mo3875getSizeYbymL2g()) + m2457getHeightimpl);
                } else if (clipBounds) {
                    bounds.intersect(0.0f, 0.0f, IntSize.m5038getWidthimpl(mo3875getSizeYbymL2g()), IntSize.m5037getHeightimpl(mo3875getSizeYbymL2g()));
                }
                if (bounds.isEmpty()) {
                    return;
                }
        ...
        float m4996getXimpl = IntOffset.m4996getXimpl(getPosition());
        bounds.setLeft(bounds.getLeft() + m4996getXimpl);
        bounds.setRight(bounds.getRight() + m4996getXimpl);
        float m4997getYimpl = IntOffset.m4997getYimpl(getPosition());
        bounds.setTop(bounds.getTop() + m4997getYimpl);
        bounds.setBottom(bounds.getBottom() + m4997getYimpl);
    }
    ...
    protected final boolean m4076withinLayerBoundsk4lQ0M(long pointerPosition) {
        if (!OffsetKt.m2408isFinitek4lQ0M(pointerPosition)) {
            return false;
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer == null || !this.isClipping || ownedLayer.mo4128isInLayerk4lQ0M(pointerPosition);
    }

    /* renamed from: isPointerInBounds-k-4lQ0M, reason: not valid java name */
    protected final boolean m4070isPointerInBoundsk4lQ0M(long pointerPosition) {
        float m2391getXimpl = Offset.m2391getXimpl(pointerPosition);
        float m2392getYimpl = Offset.m2392getYimpl(pointerPosition);
        return m2391getXimpl >= 0.0f && m2392getYimpl >= 0.0f && m2391getXimpl < ((float) getMeasuredWidth()) && m2392getYimpl < ((float) getMeasuredHeight());
    }
    ...
    static /* synthetic */ Object propagateRelocationRequest$suspendImpl(NodeCoordinator nodeCoordinator, Rect rect, Continuation<? super Unit> continuation) {
        ...
        return (nodeCoordinator2 != null && (propagateRelocationRequest = nodeCoordinator2.propagateRelocationRequest(rect.m2428translatek4lQ0M(nodeCoordinator2.localBoundingBoxOf(nodeCoordinator, false).m2426getTopLeftF1C5BW0()), continuation)) == IntrinsicsKt.getCOROUTINE_SUSPENDED()) ? propagateRelocationRequest : Unit.INSTANCE;
    }
    ...
    public final NodeCoordinator findCommonAncestor$ui_release(NodeCoordinator other) {
        ...
        if (layoutNode == layoutNode2) {
            ...
            int m4082constructorimpl = NodeKind.m4082constructorimpl(2);
            if (!tail2.getNode().getIsAttached()) {
                throw new IllegalStateException("visitLocalAncestors called on an unattached node".toString());
            }
            for (Modifier.Node parent = tail2.getNode().getParent(); parent != null; parent = parent.getParent()) {
                if ((parent.getKindSet() & m4082constructorimpl) != 0 && parent == tail) {
                    return other;
                }
            }
            return this;
    ...
    private final long m4058offsetFromEdgeMKHz9U(long pointerPosition) {
        float m2391getXimpl = Offset.m2391getXimpl(pointerPosition);
        float max = Math.max(0.0f, m2391getXimpl < 0.0f ? -m2391getXimpl : m2391getXimpl - getMeasuredWidth());
        float m2392getYimpl = Offset.m2392getYimpl(pointerPosition);
        return OffsetKt.Offset(max, Math.max(0.0f, m2392getYimpl < 0.0f ? -m2392getYimpl : m2392getYimpl - getMeasuredHeight()));
    }

    /* renamed from: calculateMinimumTouchTargetPadding-E7KxVPU, reason: not valid java name */
    protected final long m4063calculateMinimumTouchTargetPaddingE7KxVPU(long minimumTouchTargetSize) {
        return SizeKt.Size(Math.max(0.0f, (Size.m2460getWidthimpl(minimumTouchTargetSize) - getMeasuredWidth()) / 2.0f), Math.max(0.0f, (Size.m2457getHeightimpl(minimumTouchTargetSize) - getMeasuredHeight()) / 2.0f));
    }

    /* renamed from: distanceInMinimumTouchTarget-tz77jQw, reason: not valid java name */
    protected final float m4064distanceInMinimumTouchTargettz77jQw(long pointerPosition, long minimumTouchTargetSize) {
        if (getMeasuredWidth() >= Size.m2460getWidthimpl(minimumTouchTargetSize) && getMeasuredHeight() >= Size.m2457getHeightimpl(minimumTouchTargetSize)) {
            return Float.POSITIVE_INFINITY;
        }
        long m4063calculateMinimumTouchTargetPaddingE7KxVPU = m4063calculateMinimumTouchTargetPaddingE7KxVPU(minimumTouchTargetSize);
        float m2460getWidthimpl = Size.m2460getWidthimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU);
        float m2457getHeightimpl = Size.m2457getHeightimpl(m4063calculateMinimumTouchTargetPaddingE7KxVPU);
        long m4058offsetFromEdgeMKHz9U = m4058offsetFromEdgeMKHz9U(pointerPosition);
        if ((m2460getWidthimpl > 0.0f || m2457getHeightimpl > 0.0f) && Offset.m2391getXimpl(m4058offsetFromEdgeMKHz9U) <= m2460getWidthimpl && Offset.m2392getYimpl(m4058offsetFromEdgeMKHz9U) <= m2457getHeightimpl) {
            return Offset.m2390getDistanceSquaredimpl(m4058offsetFromEdgeMKHz9U);
        }
        return Float.POSITIVE_INFINITY;
    }
    ...
    public final void onMeasured() {
        ...
        if (m4055hasNodeH91voCI(NodeKind.m4082constructorimpl(128))) {
            Snapshot createNonObservableSnapshot = Snapshot.INSTANCE.createNonObservableSnapshot();
            try {
                Snapshot makeCurrent = createNonObservableSnapshot.makeCurrent();
                try {
                    int m4082constructorimpl = NodeKind.m4082constructorimpl(128);
                    boolean m4091getIncludeSelfInTraversalH91voCI = NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(m4082constructorimpl);
                    if (m4091getIncludeSelfInTraversalH91voCI) {
                        parent = getTail();
                    } else {
                        ...
                        if ((headNode.getKindSet() & m4082constructorimpl) != 0) {
                            ...
                            while (delegatingNode != 0) {
                                if (delegatingNode instanceof LayoutAwareModifierNode) {
                                    ((LayoutAwareModifierNode) delegatingNode).mo3966onRemeasuredozmzZPI(getMeasuredSize());
                                } else if ((delegatingNode.getKindSet() & m4082constructorimpl) != 0 && (delegatingNode instanceof DelegatingNode)) {
                                    Modifier.Node delegate = delegatingNode.getDelegate();
                                    int i = 0;
                                    delegatingNode = delegatingNode;
    ...
    public final void drawContainedDrawModifiers(Canvas canvas) {
        Modifier.Node m4068headH91voCI = m4068headH91voCI(NodeKind.m4082constructorimpl(4));
        if (m4068headH91voCI == null) {
            performDraw(canvas);
        } else {
            getLayoutNode().getMDrawScope$ui_release().m4017drawx_KDEd0$ui_release(canvas, IntSizeKt.m5048toSizeozmzZPI(mo3875getSizeYbymL2g()), this, m4068headH91voCI);
        }
    }
    ...
    public final boolean shouldSharePointerInputWithSiblings() {
        Modifier.Node headNode = headNode(NodeKindKt.m4091getIncludeSelfInTraversalH91voCI(NodeKind.m4082constructorimpl(16)));
        if (headNode == null) {
            return false;
        }
        Modifier.Node node = headNode;
        ...
        if ((node2.getAggregateChildKindSet() & m4082constructorimpl) != 0) {
            for (Modifier.Node child = node2.getChild(); child != null; child = child.getChild()) {
                if ((child.getKindSet() & m4082constructorimpl) != 0) {
