...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2684toArgb8_81llA(colors.get(i).m2640unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m2392getYimpl = Offset.m2391getXimpl(packedValue);
            } else {
                m2392getYimpl = Offset.m2392getYimpl(packedValue);
            }
            fArr[i] = m2392getYimpl;
        }
        return fArr;
    }
}
