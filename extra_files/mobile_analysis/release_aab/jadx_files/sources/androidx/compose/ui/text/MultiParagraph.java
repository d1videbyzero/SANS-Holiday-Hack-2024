...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m4836getMinWidthimpl(j) != 0 || Constraints.m4835getMinHeightimpl(j) != 0) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m4829getHasBoundedHeightimpl(j)) {
                m4833getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m4833getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                m4833getMaxHeightimpl = Constraints.m4833getMaxHeightimpl(j);
            }
            Paragraph m4308Paragraph_EkL_Y = ParagraphKt.m4308Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, m4834getMaxWidthimpl, 0, m4833getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + m4308Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + m4308Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(m4308Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (m4308Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m4834getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m4294paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m2666getUnspecified0d7_KjU();
        }
        multiParagraph.m4299paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m4299paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo4271paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m4298paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo4270paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m4300painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m4633drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int findParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); findParagraphByIndex < size; findParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                Path.m2895addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final int m4296getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m2392getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m2392getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m2392getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo4268getOffsetForPositionk4lQ0M(paragraphInfo.m4305toLocalMKHz9U(position)));
    }
    ...
    public final long m4297getWordBoundaryjx7JFs(int offset) {
        int findParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            findParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(findParagraphByIndex);
        return paragraphInfo.m4304toGlobalGEjPoXI(paragraphInfo.getParagraph().mo4269getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
