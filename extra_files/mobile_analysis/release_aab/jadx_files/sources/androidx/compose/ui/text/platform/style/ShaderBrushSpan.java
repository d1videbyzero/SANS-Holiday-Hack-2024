...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float f) {
        ...
        this.alpha = f;
        this.size = Size.INSTANCE.m2468getUnspecifiedNHjbRc();
    }
    ...
    public final void m4664setSizeuvyYCjk(long j) {
        this.size = j;
    }

    @Override // android.text.style.CharacterStyle
    public void updateDrawState(TextPaint textPaint) {
        Shader mo2599createShaderuvyYCjk;
        Intrinsics.checkNotNullParameter(textPaint, "textPaint");
        AndroidTextPaint_androidKt.setAlpha(textPaint, this.alpha);
        if (this.size == Size.INSTANCE.m2468getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m2456equalsimpl0(pair.getFirst().getPackedValue(), this.size)) {
            mo2599createShaderuvyYCjk = this.shaderBrush.mo2599createShaderuvyYCjk(this.size);
        } else {
            mo2599createShaderuvyYCjk = pair.getSecond();
        }
        textPaint.setShader(mo2599createShaderuvyYCjk);
        this.cachedShader = TuplesKt.to(Size.m2448boximpl(this.size), mo2599createShaderuvyYCjk);
    }
}
