...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long m4001findBestHitDistanceptXAw2c = m4001findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m3996getDistanceimpl(m4001findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m3998isInLayerimpl(m4001findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m3992compareToS_HNhKs(m4001findBestHitDistanceptXAw2c(), DistanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4001findBestHitDistanceptXAw2c() {
        long DistanceAndInLayer;
        ...
        if (i <= lastIndex) {
            while (true) {
                long m3993constructorimpl = DistanceAndInLayer.m3993constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m3992compareToS_HNhKs(m3993constructorimpl, DistanceAndInLayer) < 0) {
                    DistanceAndInLayer = m3993constructorimpl;
                }
                if (DistanceAndInLayer.m3996getDistanceimpl(DistanceAndInLayer) < 0.0f && DistanceAndInLayer.m3998isInLayerimpl(DistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m3992compareToS_HNhKs(m4001findBestHitDistanceptXAw2c, m4001findBestHitDistanceptXAw2c()) > 0) {
