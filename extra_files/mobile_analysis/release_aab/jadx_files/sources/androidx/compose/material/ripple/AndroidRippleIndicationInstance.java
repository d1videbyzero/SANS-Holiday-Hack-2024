...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> color, State<RippleAlpha> rippleAlpha, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.INSTANCE.m2469getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo3174getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m961getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3174getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo321roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m2640unboximpl = this.color.getValue().m2640unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m966drawStateLayerH2RKhps(contentDrawScope2, this.radius, m2640unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m965updateRipplePropertiesbiQXAtU(contentDrawScope.mo3174getSizeNHjbRc(), this.rippleRadius, m2640unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m964addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m2640unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
