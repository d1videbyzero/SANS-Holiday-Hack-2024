...
public final class RippleAnimation {
    ...
    */
    ...
    public final void m960draw4WTKRHQ(DrawScope draw, long j) {
        Float valueOf;
        Intrinsics.checkNotNullParameter(draw, "$this$draw");
        if (this.startRadius == null) {
            this.startRadius = Float.valueOf(RippleAnimationKt.m962getRippleStartRadiusuvyYCjk(draw.mo3174getSizeNHjbRc()));
        }
        if (this.targetRadius == null) {
            if (Float.isNaN(this.radius)) {
                valueOf = Float.valueOf(RippleAnimationKt.m961getRippleEndRadiuscSwnlzA(draw, this.bounded, draw.mo3174getSizeNHjbRc()));
            } else {
                valueOf = Float.valueOf(draw.mo327toPx0680j_4(this.radius));
            }
            this.targetRadius = valueOf;
        }
        if (this.origin == null) {
            this.origin = Offset.m2380boximpl(draw.mo3173getCenterF1C5BW0());
        }
        if (this.targetCenter == null) {
            this.targetCenter = Offset.m2380boximpl(OffsetKt.Offset(Size.m2460getWidthimpl(draw.mo3174getSizeNHjbRc()) / 2.0f, Size.m2457getHeightimpl(draw.mo3174getSizeNHjbRc()) / 2.0f));
        }
        ...
        float m2391getXimpl = Offset.m2391getXimpl(offset.getPackedValue());
        Offset offset2 = this.targetCenter;
        Intrinsics.checkNotNull(offset2);
        float lerp2 = MathHelpersKt.lerp(m2391getXimpl, Offset.m2391getXimpl(offset2.getPackedValue()), this.animatedCenterPercent.getValue().floatValue());
        Offset offset3 = this.origin;
        Intrinsics.checkNotNull(offset3);
        float m2392getYimpl = Offset.m2392getYimpl(offset3.getPackedValue());
        Offset offset4 = this.targetCenter;
        Intrinsics.checkNotNull(offset4);
        long Offset = OffsetKt.Offset(lerp2, MathHelpersKt.lerp(m2392getYimpl, Offset.m2392getYimpl(offset4.getPackedValue()), this.animatedCenterPercent.getValue().floatValue()));
        long m2629copywmQWz5c$default = Color.m2629copywmQWz5c$default(j, Color.m2632getAlphaimpl(j) * floatValue, 0.0f, 0.0f, 0.0f, 14, null);
        if (!this.bounded) {
            DrawScope.m3156drawCircleVaOC9Bg$default(draw, m2629copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
            return;
        }
        float m2460getWidthimpl = Size.m2460getWidthimpl(draw.mo3174getSizeNHjbRc());
        float m2457getHeightimpl = Size.m2457getHeightimpl(draw.mo3174getSizeNHjbRc());
        int m2619getIntersectrtfAjoo = ClipOp.INSTANCE.m2619getIntersectrtfAjoo();
        DrawContext drawContext = draw.getDrawContext();
        long mo3099getSizeNHjbRc = drawContext.mo3099getSizeNHjbRc();
        drawContext.getCanvas().save();
        drawContext.getTransform().mo3102clipRectN_I0leg(0.0f, 0.0f, m2460getWidthimpl, m2457getHeightimpl, m2619getIntersectrtfAjoo);
        DrawScope.m3156drawCircleVaOC9Bg$default(draw, m2629copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
