...
public final class SwipeableV2State<T> {
    ...
    public /* synthetic */ SwipeableV2State(Object obj, SpringSpec<Float> springSpec, AnonymousClass1 anonymousClass1, Function2 function2, float f, int i, DefaultConstructorMarker defaultConstructorMarker) {
        ...
        } : anonymousClass1, (i & 8) != 0 ? SwipeableV2Defaults.INSTANCE.getPositionalThreshold() : function2, (i & 16) != 0 ? SwipeableV2Defaults.INSTANCE.m1490getVelocityThresholdD9Ej5fM() : f, null);
    }
    ...
    */
    ...
    public final T computeTarget(float offset, T currentValue, float velocity) {
        ...
        if (f.floatValue() < offset) {
            ...
            closestAnchor = SwipeableV2Kt.closestAnchor(anchors$material3_release, offset, true);
            if (offset < Math.abs(f.floatValue() + Math.abs(this.positionalThreshold.invoke(requireDensity, Float.valueOf(Math.abs(((Number) MapsKt.getValue(anchors$material3_release, closestAnchor)).floatValue() - f.floatValue()))).floatValue()))) {
                return currentValue;
            }
        } else {
            if (velocity <= (-mo327toPx0680j_4)) {
            ...
            float abs = Math.abs(f.floatValue() - Math.abs(this.positionalThreshold.invoke(requireDensity, Float.valueOf(Math.abs(f.floatValue() - ((Number) MapsKt.getValue(anchors$material3_release, closestAnchor)).floatValue()))).floatValue()));
            if (offset < 0.0f) {
    ...
    public static final class Companion {
        ...
        public final <T> Saver<SwipeableV2State<T>, T> m1494SavereqLRuRQ(final AnimationSpec<Float> animationSpec, final Function1<? super T, Boolean> confirmValueChange, final Function2<? super Density, ? super Float, Float> positionalThreshold, final float velocityThreshold) {
            Intrinsics.checkNotNullParameter(animationSpec, "animationSpec");
            Intrinsics.checkNotNullParameter(confirmValueChange, "confirmValueChange");
            Intrinsics.checkNotNullParameter(positionalThreshold, "positionalThreshold");
