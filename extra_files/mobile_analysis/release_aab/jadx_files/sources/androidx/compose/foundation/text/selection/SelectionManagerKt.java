...
public final class SelectionManagerKt {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo, boolean z) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null) {
            ...
            if (offset > anchorSelectable$foundation_release.getLastVisibleOffset()) {
                return Offset.INSTANCE.m2406getUnspecifiedF1C5BW0();
            }
            Offset m917getCurrentDragPosition_m7T9E = selectionManager.m917getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(m917getCurrentDragPosition_m7T9E);
            float m2391getXimpl = Offset.m2391getXimpl(layoutCoordinates.mo3876localPositionOfR5De75A(containerLayoutCoordinates, m917getCurrentDragPosition_m7T9E.getPackedValue()));
            long mo878getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo878getRangeOfLineContainingjx7JFs(offset);
            Rect boundingBox = anchorSelectable$foundation_release.getBoundingBox(TextRange.m4406getMinimpl(mo878getRangeOfLineContainingjx7JFs));
            Rect boundingBox2 = anchorSelectable$foundation_release.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m4405getMaximpl(mo878getRangeOfLineContainingjx7JFs) - 1, TextRange.m4406getMinimpl(mo878getRangeOfLineContainingjx7JFs)));
            float coerceIn = RangesKt.coerceIn(m2391getXimpl, Math.min(boundingBox.getLeft(), boundingBox2.getLeft()), Math.max(boundingBox.getRight(), boundingBox2.getRight()));
            if (Math.abs(m2391getXimpl - coerceIn) > IntSize.m5038getWidthimpl(j) / 2) {
                return Offset.INSTANCE.m2406getUnspecifiedF1C5BW0();
            }
            return containerLayoutCoordinates.mo3876localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(coerceIn, Offset.m2392getYimpl(anchorSelectable$foundation_release.getBoundingBox(offset).m2421getCenterF1C5BW0())));
        }
        return Offset.INSTANCE.m2406getUnspecifiedF1C5BW0();
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m928calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long j) {
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m2429Rect0a9Yr6o(layoutCoordinates.mo3880windowToLocalMKHz9U(boundsInWindow.m2426getTopLeftF1C5BW0()), layoutCoordinates.mo3880windowToLocalMKHz9U(boundsInWindow.m2420getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m929containsInclusiveUv8p0NA(Rect containsInclusive, long j) {
