...
public interface SelectionAdjustment {
    ...
    long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange);
    ...
    public static final class Companion {
        ...
        private static final SelectionAdjustment None = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$None$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
        ...
        private static final SelectionAdjustment Character = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Character$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                if (TextRange.m4402getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4408getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, previousSelectionRange != null ? TextRange.m4407getReversedimpl(previousSelectionRange.getPackedValue()) : false);
                }
                return newRawSelectionRange;
            }
        };
        private static final SelectionAdjustment Word = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Word$1
            ...
            /* renamed from: adjust-ZXO7KMw */
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m889adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m889adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m889adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Word$1$adjust$1(textLayoutResult));
                return m889adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment Paragraph = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Paragraph$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m889adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m889adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m889adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Paragraph$1$adjust$boundaryFun$1(textLayoutResult.getLayoutInput().getText()));
                return m889adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment CharacterWithWordAccelerate = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$CharacterWithWordAccelerate$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                if (previousSelectionRange == null) {
                    return SelectionAdjustment.Companion.$$INSTANCE.getWord().mo887adjustZXO7KMw(textLayoutResult, newRawSelectionRange, previousHandleOffset, isStartHandle, previousSelectionRange);
                }
                if (TextRange.m4402getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4408getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, TextRange.m4407getReversedimpl(previousSelectionRange.getPackedValue()));
                }
                if (isStartHandle) {
                    i = updateSelectionBoundary(textLayoutResult, TextRange.m4408getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4408getStartimpl(previousSelectionRange.getPackedValue()), TextRange.m4403getEndimpl(newRawSelectionRange), true, TextRange.m4407getReversedimpl(newRawSelectionRange));
                    updateSelectionBoundary = TextRange.m4403getEndimpl(newRawSelectionRange);
                } else {
                    int m4408getStartimpl = TextRange.m4408getStartimpl(newRawSelectionRange);
                    updateSelectionBoundary = updateSelectionBoundary(textLayoutResult, TextRange.m4403getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4403getEndimpl(previousSelectionRange.getPackedValue()), TextRange.m4408getStartimpl(newRawSelectionRange), false, TextRange.m4407getReversedimpl(newRawSelectionRange));
                    i = m4408getStartimpl;
                }
            ...
            private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
                int lineStart;
                int lineEnd$default;
                long m4382getWordBoundaryjx7JFs = textLayoutResult.m4382getWordBoundaryjx7JFs(newRawOffset);
                if (textLayoutResult.getLineForOffset(TextRange.m4408getStartimpl(m4382getWordBoundaryjx7JFs)) == currentLine) {
                    lineStart = TextRange.m4408getStartimpl(m4382getWordBoundaryjx7JFs);
                } else {
                    lineStart = textLayoutResult.getLineStart(currentLine);
                }
                if (textLayoutResult.getLineForOffset(TextRange.m4403getEndimpl(m4382getWordBoundaryjx7JFs)) == currentLine) {
                    lineEnd$default = TextRange.m4403getEndimpl(m4382getWordBoundaryjx7JFs);
                } else {
            ...
            private final boolean isAtWordBoundary(TextLayoutResult textLayoutResult, int i) {
                long m4382getWordBoundaryjx7JFs = textLayoutResult.m4382getWordBoundaryjx7JFs(i);
                return i == TextRange.m4408getStartimpl(m4382getWordBoundaryjx7JFs) || i == TextRange.m4403getEndimpl(m4382getWordBoundaryjx7JFs);
            }
        };
        ...
        public final long m889adjustByBoundaryDvylE(TextLayoutResult textLayoutResult, long newRawSelection, Function1<? super Integer, TextRange> boundaryFun) {
            if (textLayoutResult.getLayoutInput().getText().length() == 0) {
                return TextRange.INSTANCE.m4413getZerod9O1mEE();
            }
            int lastIndex = StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText());
            long packedValue = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4408getStartimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            long packedValue2 = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4403getEndimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            return TextRangeKt.TextRange(TextRange.m4407getReversedimpl(newRawSelection) ? TextRange.m4403getEndimpl(packedValue) : TextRange.m4408getStartimpl(packedValue), TextRange.m4407getReversedimpl(newRawSelection) ? TextRange.m4408getStartimpl(packedValue2) : TextRange.m4403getEndimpl(packedValue2));
        }
