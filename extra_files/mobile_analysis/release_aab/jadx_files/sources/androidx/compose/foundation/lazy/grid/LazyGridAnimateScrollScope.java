...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            m4996getXimpl = IntOffset.m4997getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            m4996getXimpl = IntOffset.m4996getXimpl(lazyGridItemInfo2.getOffset());
        }
        return Integer.valueOf(m4996getXimpl);
    }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (isVertical) {
                        m5038getWidthimpl = IntSize.m5037getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        m5038getWidthimpl = IntSize.m5038getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    i4 = Math.max(i4, m5038getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
