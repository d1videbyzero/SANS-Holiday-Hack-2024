...
public final class TextFieldSelectionManagerKt {
    ...
    public static final boolean isSelectionHandleInVisibleBound(TextFieldSelectionManager textFieldSelectionManager, boolean z) {
        ...
        return SelectionManagerKt.m929containsInclusiveUv8p0NA(visibleBounds, textFieldSelectionManager.m948getHandlePositiontuRUvjQ$foundation_release(z));
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m949calculateSelectionMagnifierCenterAndroidO0kMr_c(TextFieldSelectionManager manager, long j) {
        int m4408getStartimpl;
        ...
        if (i == 1 || i == 2) {
            m4408getStartimpl = TextRange.m4408getStartimpl(manager.getValue$foundation_release().getSelection());
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            m4408getStartimpl = TextRange.m4403getEndimpl(manager.getValue$foundation_release().getSelection());
        }
        int originalToTransformed = manager.getOffsetMapping().originalToTransformed(m4408getStartimpl);
        ...
        long m2421getCenterF1C5BW0 = value.getBoundingBox(coerceIn).m2421getCenterF1C5BW0();
        ...
        Offset m946getCurrentDragPosition_m7T9E = manager.m946getCurrentDragPosition_m7T9E();
        if (m946getCurrentDragPosition_m7T9E == null) {
            return Offset.INSTANCE.m2406getUnspecifiedF1C5BW0();
        }
        float m2391getXimpl = Offset.m2391getXimpl(innerTextFieldCoordinates.mo3876localPositionOfR5De75A(layoutCoordinates, m946getCurrentDragPosition_m7T9E.getPackedValue()));
        ...
        boolean z = TextRange.m4408getStartimpl(manager.getValue$foundation_release().getSelection()) > TextRange.m4403getEndimpl(manager.getValue$foundation_release().getSelection());
        ...
        float coerceIn2 = RangesKt.coerceIn(m2391getXimpl, Math.min(horizontalPosition, horizontalPosition2), Math.max(horizontalPosition, horizontalPosition2));
        if (Math.abs(m2391getXimpl - coerceIn2) > IntSize.m5038getWidthimpl(j) / 2) {
            return Offset.INSTANCE.m2406getUnspecifiedF1C5BW0();
        }
        return layoutCoordinates.mo3876localPositionOfR5De75A(innerTextFieldCoordinates, OffsetKt.Offset(coerceIn2, Offset.m2392getYimpl(m2421getCenterF1C5BW0)));
    }
}
