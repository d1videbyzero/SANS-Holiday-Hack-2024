...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5037getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m5037getHeightimpl(textLayoutResult.getSize())) {
                    lineCount--;
                }
    ...
    public Pair<Selection, Boolean> mo879updateSelectionqCDeeow(long startHandlePosition, long endHandlePosition, Offset previousHandlePosition, boolean isStartHandle, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection previousSelection) {
        ...
        if (layoutCoordinates != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            long mo3876localPositionOfR5De75A = containerLayoutCoordinates.mo3876localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m2407getZeroF1C5BW0());
            return MultiWidgetSelectionDelegateKt.m883getTextSelectionInfoyM0VcXU(invoke, Offset.m2395minusMKHz9U(startHandlePosition, mo3876localPositionOfR5De75A), Offset.m2395minusMKHz9U(endHandlePosition, mo3876localPositionOfR5De75A), previousHandlePosition != null ? Offset.m2380boximpl(Offset.m2395minusMKHz9U(previousHandlePosition.getPackedValue(), mo3876localPositionOfR5De75A)) : null, getSelectableId(), adjustment, previousSelection, isStartHandle);
        }
    ...
    public Selection getSelectAllSelection() {
        Selection m881getAssembledSelectionInfovJH6DeI;
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
            return null;
        }
        m881getAssembledSelectionInfovJH6DeI = MultiWidgetSelectionDelegateKt.m881getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, invoke.getLayoutInput().getText().length()), false, getSelectableId(), invoke);
        return m881getAssembledSelectionInfovJH6DeI;
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU, reason: not valid java name */
    public long mo877getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult invoke;
    ...
    public long mo878getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
