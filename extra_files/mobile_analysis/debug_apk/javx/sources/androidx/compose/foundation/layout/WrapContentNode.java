...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo239measure3p2s80s(final MeasureScope measure, Measurable measurable, long constraints) {
        ...
        int m5174getMinWidthimpl = this.direction != Direction.Vertical ? 0 : Constraints.m5174getMinWidthimpl(constraints);
        int m5173getMinHeightimpl = this.direction == Direction.Horizontal ? Constraints.m5173getMinHeightimpl(constraints) : 0;
        if (this.direction != Direction.Vertical && this.unbounded) {
            m5172getMaxWidthimpl = Integer.MAX_VALUE;
        } else {
            m5172getMaxWidthimpl = Constraints.m5172getMaxWidthimpl(constraints);
        }
        long wrappedConstraints = ConstraintsKt.Constraints(m5174getMinWidthimpl, m5172getMaxWidthimpl, m5173getMinHeightimpl, (this.direction == Direction.Horizontal || !this.unbounded) ? Constraints.m5171getMaxHeightimpl(constraints) : Integer.MAX_VALUE);
        final Placeable placeable = measurable.mo4184measureBRTryo0(wrappedConstraints);
        final int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m5174getMinWidthimpl(constraints), Constraints.m5172getMaxWidthimpl(constraints));
        final int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m5173getMinHeightimpl(constraints), Constraints.m5171getMaxHeightimpl(constraints));
        return MeasureScope.layout$default(measure, wrapperWidth, wrapperHeight, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.WrapContentNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope layout) {
                ...
                long position = WrapContentNode.this.getAlignmentCallback().invoke(IntSize.m5368boximpl(IntSizeKt.IntSize(wrapperWidth - placeable.getWidth(), wrapperHeight - placeable.getHeight())), measure.getLayoutDirection()).getPackedValue();
                Placeable.PlacementScope.m4241place70tqf50$default(layout, placeable, position, 0.0f, 2, null);
            }
        }, 4, null);
    }
}
