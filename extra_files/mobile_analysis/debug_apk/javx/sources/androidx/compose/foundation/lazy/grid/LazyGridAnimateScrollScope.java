...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (item != null) {
            return Integer.valueOf(this.state.getIsVertical() ? IntOffset.m5335getYimpl(item.getOffset()) : IntOffset.m5334getXimpl(item.getOffset()));
        }
        return null;
    }
        ...
        while (lineStartIndex < visibleItems.size()) {
            ...
            if (currentLine == -1) {
                ...
                while (lineEndIndex < visibleItems.size() && lineOf.invoke(Integer.valueOf(lineEndIndex)).intValue() == currentLine) {
                    if (isVertical) {
                        m5376getWidthimpl = IntSize.m5375getHeightimpl(visibleItems.get(lineEndIndex).getSize());
                    } else {
                        m5376getWidthimpl = IntSize.m5376getWidthimpl(visibleItems.get(lineEndIndex).getSize());
                    }
                    lineMainAxisSize = Math.max(lineMainAxisSize, m5376getWidthimpl);
                    lineEndIndex++;
                }
