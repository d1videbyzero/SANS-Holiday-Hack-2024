...
public abstract class LazyGridMeasuredItemProvider {
    ...
    public final LazyGridMeasuredItem m621getAndMeasure3p2s80s(int index, int mainAxisSpacing, long constraints) {
        ...
        List placeables = this.measureScope.mo640measure0kLqBqw(index, constraints);
        if (Constraints.m5170getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m5174getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m5169getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("Failed requirement.".toString());
            }
            crossAxisSize = Constraints.m5173getMinHeightimpl(constraints);
        }
