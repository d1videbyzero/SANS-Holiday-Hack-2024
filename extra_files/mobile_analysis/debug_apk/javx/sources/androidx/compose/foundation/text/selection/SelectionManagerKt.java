...
public final class SelectionManagerKt {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager $manager, long $magnifierSize, Selection.AnchorInfo anchor, boolean isStartHandle) {
        ...
        if (selectable != null && (containerCoordinates = $manager.getContainerLayoutCoordinates()) != null && (selectableCoordinates = selectable.getLayoutCoordinates()) != null) {
            ...
            Offset m917getCurrentDragPosition_m7T9E = $manager.m917getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(m917getCurrentDragPosition_m7T9E);
            long localDragPosition = selectableCoordinates.mo4192localPositionOfR5De75A(containerCoordinates, m917getCurrentDragPosition_m7T9E.getPackedValue());
            float dragX = Offset.m2708getXimpl(localDragPosition);
            long line = selectable.mo878getRangeOfLineContainingjx7JFs(offset);
            Rect lineMin = selectable.getBoundingBox(TextRange.m4722getMinimpl(line));
            Rect lineMax = selectable.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m4721getMaximpl(line) - 1, TextRange.m4722getMinimpl(line)));
            ...
            if (Math.abs(dragX - maxX) > IntSize.m5376getWidthimpl($magnifierSize) / 2) {
                return Offset.INSTANCE.m2723getUnspecifiedF1C5BW0();
            }
            Rect anchorBounds = selectable.getBoundingBox(offset);
            float centerY = Offset.m2709getYimpl(anchorBounds.m2738getCenterF1C5BW0());
            return containerCoordinates.mo4192localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(maxX, centerY));
        }
        return Offset.INSTANCE.m2723getUnspecifiedF1C5BW0();
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m928calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long magnifierSize) {
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m2746Rect0a9Yr6o($this$visibleBounds.mo4196windowToLocalMKHz9U(boundsInWindow.m2743getTopLeftF1C5BW0()), $this$visibleBounds.mo4196windowToLocalMKHz9U(boundsInWindow.m2737getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m929containsInclusiveUv8p0NA(Rect containsInclusive, long offset) {
        Intrinsics.checkNotNullParameter(containsInclusive, "$this$containsInclusive");
        float left = containsInclusive.getLeft();
        float right = containsInclusive.getRight();
        float m2708getXimpl = Offset.m2708getXimpl(offset);
        if (left <= m2708getXimpl && m2708getXimpl <= right) {
            float top = containsInclusive.getTop();
            float bottom = containsInclusive.getBottom();
            float m2709getYimpl = Offset.m2709getYimpl(offset);
            if (top <= m2709getYimpl && m2709getYimpl <= bottom) {
                return true;
            }
        }
        return false;
