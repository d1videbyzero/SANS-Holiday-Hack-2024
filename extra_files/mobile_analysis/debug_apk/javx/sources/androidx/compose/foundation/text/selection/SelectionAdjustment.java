...
public interface SelectionAdjustment {
    ...
    public static final class Companion {
        ...
        private static final SelectionAdjustment Character = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Character$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                if (TextRange.m4718getCollapsedimpl(newRawSelectionRange)) {
                    boolean previousHandlesCrossed = previousSelectionRange != null ? TextRange.m4723getReversedimpl(previousSelectionRange.getPackedValue()) : false;
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4724getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, previousHandlesCrossed);
                }
                return newRawSelectionRange;
            }
        };
        ...
        private static final SelectionAdjustment Paragraph = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Paragraph$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                m889adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m889adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, (Function1) boundaryFun);
                return m889adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment CharacterWithWordAccelerate = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$CharacterWithWordAccelerate$1
            ...
            public long mo887adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                if (TextRange.m4718getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4724getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, TextRange.m4723getReversedimpl(previousSelectionRange.getPackedValue()));
                }
                if (isStartHandle) {
                    start = updateSelectionBoundary(textLayoutResult, TextRange.m4724getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4724getStartimpl(previousSelectionRange.getPackedValue()), TextRange.m4719getEndimpl(newRawSelectionRange), true, TextRange.m4723getReversedimpl(newRawSelectionRange));
                    end = TextRange.m4719getEndimpl(newRawSelectionRange);
                } else {
                    start = TextRange.m4724getStartimpl(newRawSelectionRange);
                    end = updateSelectionBoundary(textLayoutResult, TextRange.m4719getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4719getEndimpl(previousSelectionRange.getPackedValue()), TextRange.m4724getStartimpl(newRawSelectionRange), false, TextRange.m4723getReversedimpl(newRawSelectionRange));
                }
            ...
            private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
                int start;
                int end;
                long wordBoundary = textLayoutResult.m4698getWordBoundaryjx7JFs(newRawOffset);
                int wordStartLine = textLayoutResult.getLineForOffset(TextRange.m4724getStartimpl(wordBoundary));
                if (wordStartLine == currentLine) {
                    start = TextRange.m4724getStartimpl(wordBoundary);
                } else {
                    start = textLayoutResult.getLineStart(currentLine);
                }
                int wordEndLine = textLayoutResult.getLineForOffset(TextRange.m4719getEndimpl(wordBoundary));
                if (wordEndLine != currentLine) {
                    end = TextLayoutResult.getLineEnd$default(textLayoutResult, currentLine, false, 2, null);
                } else {
                    end = TextRange.m4719getEndimpl(wordBoundary);
                }
            ...
            private final boolean isAtWordBoundary(TextLayoutResult $this$isAtWordBoundary, int offset) {
                long wordBoundary = $this$isAtWordBoundary.m4698getWordBoundaryjx7JFs(offset);
                return offset == TextRange.m4724getStartimpl(wordBoundary) || offset == TextRange.m4719getEndimpl(wordBoundary);
            }
        ...
        public final long m889adjustByBoundaryDvylE(TextLayoutResult textLayoutResult, long newRawSelection, Function1<? super Integer, TextRange> boundaryFun) {
            if (textLayoutResult.getLayoutInput().getText().length() == 0) {
                return TextRange.INSTANCE.m4729getZerod9O1mEE();
            }
            int maxOffset = StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText());
            long startBoundary = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4724getStartimpl(newRawSelection), 0, maxOffset))).getPackedValue();
            long endBoundary = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4719getEndimpl(newRawSelection), 0, maxOffset))).getPackedValue();
            int start = TextRange.m4723getReversedimpl(newRawSelection) ? TextRange.m4719getEndimpl(startBoundary) : TextRange.m4724getStartimpl(startBoundary);
            int end = TextRange.m4723getReversedimpl(newRawSelection) ? TextRange.m4724getStartimpl(endBoundary) : TextRange.m4719getEndimpl(endBoundary);
            return TextRangeKt.TextRange(start, end);
        }
