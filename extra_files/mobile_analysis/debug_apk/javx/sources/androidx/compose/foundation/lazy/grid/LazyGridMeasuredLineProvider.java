...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m622childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m5182fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.INSTANCE.m5181fixedHeightOenEA2s(crossAxisSize);
    }
    ...
    public final long m623itemConstraintsOenEA2s(int itemIndex) {
        ...
        return m622childConstraintsJhjzzOo$foundation_release(0, span);
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m601getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).getPackedValue());
            long constraints = m622childConstraintsJhjzzOo$foundation_release(startSlot, span);
            LazyGridMeasuredItem m621getAndMeasure3p2s80s = this.measuredItemProvider.m621getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i, mainAxisSpacing, constraints);
            startSlot += span;
            Unit unit = Unit.INSTANCE;
            items[i] = m621getAndMeasure3p2s80s;
        }
