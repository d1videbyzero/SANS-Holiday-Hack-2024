    ...
    */
    ...
    public final void m1277draw4WTKRHQ(DrawScope draw, long color) {
        ...
        if (this.startRadius == null) {
            this.startRadius = Float.valueOf(RippleAnimationKt.m1279getRippleStartRadiusuvyYCjk(draw.mo3490getSizeNHjbRc()));
        }
        if (this.targetRadius == null) {
            float $this$isUnspecified$iv = this.radius;
            this.targetRadius = Float.isNaN($this$isUnspecified$iv) ? Float.valueOf(RippleAnimationKt.m1278getRippleEndRadiuscSwnlzA(draw, this.bounded, draw.mo3490getSizeNHjbRc())) : Float.valueOf(draw.mo327toPx0680j_4(this.radius));
        }
        if (this.origin == null) {
            this.origin = Offset.m2697boximpl(draw.mo3489getCenterF1C5BW0());
        }
        if (this.targetCenter == null) {
            this.targetCenter = Offset.m2697boximpl(OffsetKt.Offset(Size.m2777getWidthimpl(draw.mo3490getSizeNHjbRc()) / 2.0f, Size.m2774getHeightimpl(draw.mo3490getSizeNHjbRc()) / 2.0f));
        }
        ...
        float m2708getXimpl = Offset.m2708getXimpl(offset.getPackedValue());
        Offset offset2 = this.targetCenter;
        Intrinsics.checkNotNull(offset2);
        float lerp = MathHelpersKt.lerp(m2708getXimpl, Offset.m2708getXimpl(offset2.getPackedValue()), this.animatedCenterPercent.getValue().floatValue());
        Offset offset3 = this.origin;
        Intrinsics.checkNotNull(offset3);
        float m2709getYimpl = Offset.m2709getYimpl(offset3.getPackedValue());
        Offset offset4 = this.targetCenter;
        Intrinsics.checkNotNull(offset4);
        long centerOffset = OffsetKt.Offset(lerp, MathHelpersKt.lerp(m2709getYimpl, Offset.m2709getYimpl(offset4.getPackedValue()), this.animatedCenterPercent.getValue().floatValue()));
        modulatedColor = Color.m2945copywmQWz5c(color, (r12 & 1) != 0 ? Color.m2949getAlphaimpl(color) : Color.m2949getAlphaimpl(color) * alpha, (r12 & 2) != 0 ? Color.m2953getRedimpl(color) : 0.0f, (r12 & 4) != 0 ? Color.m2952getGreenimpl(color) : 0.0f, (r12 & 8) != 0 ? Color.m2950getBlueimpl(color) : 0.0f);
        if (!this.bounded) {
            DrawScope.m3472drawCircleVaOC9Bg$default(draw, modulatedColor, radius, centerOffset, 0.0f, null, null, 0, 120, null);
            return;
        }
        float right$iv = Size.m2777getWidthimpl(draw.mo3490getSizeNHjbRc());
        float bottom$iv = Size.m2774getHeightimpl(draw.mo3490getSizeNHjbRc());
        ...
        $this$clipRect_rOu3jXo_u24lambda_u244$iv.mo3418clipRectN_I0leg(0.0f, 0.0f, right$iv, bottom$iv, clipOp$iv);
        DrawScope.m3472drawCircleVaOC9Bg$default(draw, modulatedColor, radius, centerOffset, 0.0f, null, null, 0, 120, null);
