...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m4284constructorimpl(new int[(oldEnd * 2) + 1]);
        int[] backward = CenteredArray.m4284constructorimpl(new int[(oldEnd * 2) + 1]);
    ...
    private static final boolean m4359midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        ...
        CenteredArray.m4290setimpl(forward, 1, oldStart);
        CenteredArray.m4290setimpl(backward, 1, oldEnd);
        int d = 0;
        while (d < max) {
            int d2 = d;
    ...
    private static final boolean m4358forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        ...
        int k = -d;
        while (k <= d) {
            if (k == (-d) || (k != d && CenteredArray.m4287getimpl(forward, k + 1) > CenteredArray.m4287getimpl(forward, k - 1))) {
                startX = CenteredArray.m4287getimpl(forward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4287getimpl(forward, k - 1);
                x = startX + 1;
            ...
            CenteredArray.m4290setimpl(forward, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK >= (-d) + i2 && backwardsK <= d - 1) {
                    if (CenteredArray.m4287getimpl(backward, backwardsK) > x) {
                        i2 = 1;
                    } else {
    ...
    private static final boolean m4357backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        ...
        int delta = oldSize - newSize;
        for (int k = -d; k <= d; k += 2) {
            if (k == (-d) || (k != d && CenteredArray.m4287getimpl(backward, k + 1) < CenteredArray.m4287getimpl(backward, k - 1))) {
                startX = CenteredArray.m4287getimpl(backward, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m4287getimpl(backward, k - 1);
                x = startX - 1;
            ...
            CenteredArray.m4290setimpl(backward, k, x);
            if (checkForSnake) {
                int forwardsK = delta - k;
                if (forwardsK >= (-d) && forwardsK <= d && CenteredArray.m4287getimpl(forward, forwardsK) >= x) {
