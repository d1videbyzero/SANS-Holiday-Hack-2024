...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long distance = m4317findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4312getDistanceimpl(distance) < 0.0f && DistanceAndInLayer.m4314isInLayerimpl(distance);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        long bestDistance = m4317findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4308compareToS_HNhKs(bestDistance, distanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4317findBestHitDistanceptXAw2c() {
        long bestDistance;
        ...
        if (i <= lastIndex) {
            while (true) {
                long distance = DistanceAndInLayer.m4309constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                bestDistance = DistanceAndInLayer.m4308compareToS_HNhKs(distance, bestDistance) < 0 ? distance : bestDistance;
                if (DistanceAndInLayer.m4312getDistanceimpl(bestDistance) >= 0.0f || !DistanceAndInLayer.m4314isInLayerimpl(bestDistance)) {
                    if (i == lastIndex) {
                        break;
                    }
                    i++;
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        long previousDistance = m4317findBestHitDistanceptXAw2c();
        int previousHitDepth = this.hitDepth;
        this.hitDepth = CollectionsKt.getLastIndex(this);
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(this) && DistanceAndInLayer.m4308compareToS_HNhKs(previousDistance, m4317findBestHitDistanceptXAw2c()) > 0) {
